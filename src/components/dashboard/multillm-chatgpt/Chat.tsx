...
import { TypeMultiLLmChatGPT } from '@/types/types';
...
import ModalLimitExceeded from '@/components/dashboard/ModalLimitExceeded';
import { saveChat } from '@/app/dashboard/(apps)/multillm-chatgpt/actions';

interface ChatBoardProps {
  userName: string;
  chat?: TypeMultiLLmChatGPT;
}
...
const ChatBoard: FC<ChatBoardProps> = ({ userName, chat }) => {
  ...
  const [chatModel, setChatModel] = useState<string>(chat?.model || modelOptions[0].value);
  ...
  const handleChatFn = async (formData: FormData) => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch('/api/multillm-chatgpt/chat', {
        method: 'POST',
      ...
      if (output) {
        const chatResponse = await saveChat({
          id: chat?.id,
          title: chat?.title,
          model: chatModel,
        ...
        router.replace(`/dashboard/multillm-chatgpt/${chatResponse?.id}`);
        router.refresh();
      }
      setIsLoading(false);
    } catch (error) {
  ...
  return (
    <div>
      ...
      <SelectChatModel
        ...
        onSelect={(value) => setChatModel(value)}
        isDisabled={historyArray.length > 0}
      />
