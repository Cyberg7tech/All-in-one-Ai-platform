...
export default function WorkflowsPage() {
  ...
  const toggleWorkflowStatus = (workflowId: string) => {
    setWorkflows((prev) =>
      prev.map((w) =>
        w.id === workflowId ? { ...w, status: w.status === 'active' ? 'paused' : 'active' } : w
      )
    );
  };
            ...
            <Card>
              ...
              <CardContent>
                ...
                <div className='flex justify-between mt-4'>
                  ...
                  <Button onClick={saveWorkflow} disabled={!selectedTrigger || builderSteps.length === 0}>
                    Save Workflow
                  </Button>
                </div>
              </CardContent>
                    ...
                    <div className='flex items-center space-x-4'>
                      ...
                      <div>
                        ...
                        <p className='text-sm text-muted-foreground'>
                          {execution.timestamp.toLocaleString()} • {execution.duration} • {execution.steps}{' '}
                          steps
                        </p>
                        {execution.error && <p className='text-sm text-red-600'>{execution.error}</p>}
                      </div>
                    </div>
                    <Badge
                      className={
