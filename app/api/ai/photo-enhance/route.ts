...
export async function POST(request: NextRequest) {
  try {
    ...
    const { prompt, enhancement_type = 'auto-enhance', settings = {}, userId } = body;
    ...
    try {
      ...
      try {
        ...
        enhancementData = {
          ...
          enhancement_plan: {
            ...
            color_corrections: {
              brightness:
                enhancement_type === 'portrait' ? '+10-15% for better skin tones' : '+5-10% for natural look',
              contrast:
                enhancement_type === 'landscape' ? '+20-25% for dramatic effect' : '+10-15% for clarity',
            ...
            technical_settings: {
              sharpness:
                enhancement_type === 'portrait'
                  ? 'Medium (avoid over-sharpening skin)'
                  : 'High for landscape details',
          ...
          style_recommendations: {
            mood:
              enhancement_type === 'vintage'
                ? 'Nostalgic and warm'
                : enhancement_type === 'dramatic'
                  ? 'Bold and striking'
                  : 'Natural and vibrant',
            tone_mapping:
              enhancement_type === 'warm'
                ? 'Warm tones for cozy feeling'
                : enhancement_type === 'cool'
                  ? 'Cool tones for modern look'
                  : 'Balanced neutral tones',
            artistic_effects:
              enhancement_type === 'artistic' ? ['Selective color', 'Vignette'] : ['Clarity', 'Vibrance'],
          },
          before_after_expectations: response.content.includes('expect')
            ? response.content.substring(
                response.content.indexOf('expect'),
                response.content.indexOf('expect') + 200
              )
      ...
      return NextResponse.json({
        ...
        enhancement_description:
          enhancementData.analysis || `Professional ${enhancement_type} enhancement applied`,
