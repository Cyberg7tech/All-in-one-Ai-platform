...
export async function GET() {
  try {
    ...
    const enhancedModels = availableModels.map(model => ({
      ...
      tier: model.provider === 'aimlapi' ? 'premium' : 
            model.provider === 'openai' ? 'standard' : 'open-source',
      ...
      contextWindow: model.id.includes('gpt-4') ? 128000 :
                   model.id.includes('claude') ? 200000 :
                   model.id.includes('gemini') ? 1000000 :
                   model.id.includes('llama') ? 131072 :
                   32768,
                   
      // Add speed rating
      speed: model.provider === 'together' ? 'fast' :
             model.id.includes('mini') || model.id.includes('flash') ? 'fast' :
             model.id.includes('o1') || model.id.includes('opus') ? 'slow' :
             'medium'
    }));
