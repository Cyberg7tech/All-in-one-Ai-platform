...
import { Search, Plus, Edit, Trash2, Copy, Star, MessageSquare, Bot, Image, Video, Music, FileText } from 'lucide-react';
...
const defaultPrompts: Prompt[] = [
  {
    ...
    content: 'You are a professional content writer. Write engaging, SEO-optimized content about {{topic}} for {{audience}}. The content should be {{tone}} and approximately {{wordCount}} words.',
    category: 'writing',
  ...
  },
  {
    ...
    content: 'Create a detailed, high-quality image description for {{subject}} in {{style}} style. Include lighting, composition, colors, and mood. Make it suitable for AI image generation.',
    category: 'image',
  ...
  },
  {
    ...
    content: 'You are an expert programmer. Help me write {{language}} code for {{task}}. Include comments and best practices. The code should be {{complexity}} level.',
    category: 'code',
  ...
  },
  {
    ...
    content: 'You are a data analyst. Analyze the following data about {{dataset}} and provide insights. Focus on {{focusArea}} and present findings in a clear, actionable format.',
    category: 'analysis',
...
export function PromptManager({
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
      ...
      case 'analysis': return 'bg-teal-100 text-teal-800';
...
function PromptForm({ prompt, onSave, onCancel }: PromptFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newPrompt: Prompt = {
      ...
      variables: formData.variables.split(',').map(variable => variable.trim()).filter(Boolean),
